...
The rule here is: pick the newest filing for a given (issuer, insider, date,
code, ownership form), then total that filing's rows. Direct and indirect
purchases stay separate rows because they are different holdings, not tranches
of one order.
"""
...
WITH ranked AS (
    SELECT
        ...
        first_value(t.filing_id) OVER (
            PARTITION BY f.cik, t.insider_name, t.transaction_date,
                         t.transaction_code, t.is_direct
            ORDER BY f.filed_date DESC, t.filing_id DESC
